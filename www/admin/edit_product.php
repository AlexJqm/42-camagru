	...
	if (!isset($_SERVER['PHP_AUTH_USER'])) {
		...
		if ($_SERVER['PHP_AUTH_USER'] == "admin" && hash("sha256", $_SERVER['PHP_AUTH_PW']) == "8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918") {
			...
			if (isset($_GET['edit_product'])) {
				...
				$get_pro = "select * from produits where produit_id='$get_id'";
				...
				$category_titre = $row_cat['cat_titre'];
			}
?>
<html>
	<head>
	...
	<body>
		<form action="" method="post" enctype="multipart/form-data">
			<table >
				...
				<tr>
					<td><input type="submit" name="update_produit" value="Envoyer"/></td>
				</tr>
			</table>
		</form>
	</body>
...
<?php
			if (isset($_POST['update_produit'])) {
				...
				$update_produit = "update produits set produit_cat='$produit_cat',produit_titre='$produit_titre',produit_prix='$produit_prix',produit_desc='$produit_desc' where produit_id='$update_id'";
				$run_produit = mysqli_query($con, $update_produit);
				if ($run_produit)
